...
parser.add_argument('-s', '--server', help='IP address of Nessus API endpoint', default=sunucu)
...
parser.add_argument('--insecure', help='boldly go forth and ignore cert errors', action='store_true', default=True)
group = parser.add_mutually_exclusive_group()
group.add_argument('-k', '--keyfile', help='API key file in json format')
group.add_argument('-u', '--user', help='Nessus user instead of API key (password prompt will occur)',default=kullanici)
...
if not args.capath:
    if localhost:
        ...
        print(
            'WARNING:  No CA path explicitly set. Connection to ' + nessus_url + ' could fail due to TLS/SSL server authentication')

scanner = None
if keys:
    scanner = ness6rest.Scanner(url=nessus_url, api_akey=keys['accessKey'], api_skey=keys['secretKey'],
                                insecure=insecure, ca_bundle=args.capath)
elif password:
    scanner = ness6rest.Scanner(url=nessus_url, login=args.user, password=password, insecure=insecure,
...
if scanner:
    ...
    #folders = [{'name': 'My Scans', 'unread_count': 0, 'default_tag': 1, 'custom': 0, 'id': 3, 'type': 'main'}]
    ...
    for s in scans:
        ...
        # skip trash items?
        if folder_name != 'My Scans' and not args.trash:
            continue
